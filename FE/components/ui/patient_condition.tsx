...
export default function PatientCondition({ patient }: { patient: Patient }) {
  ...
  const startTime =
    Date.now() - (patient.patient_score.length - 1) * intervalMs;
  ...
  const initialTrendData = patient.patient_score
    .map((score, i) => ({
  ...
  const initialWeightData = patient.patient_score_fields.body_weight_change
    .map((value, i) => ({
      time: formatTime(startTime + i * intervalMs),
      value
    }))
  ...
  const initialHydrationData = patient.patient_score_fields.hydration_levels
    .map((value, i) => ({
      time: formatTime(startTime + i * intervalMs),
      value
    }))
  ...
  const initialSleepData = patient.patient_score_fields.sleep_deprivation
    .map((value, i) => ({
      time: formatTime(startTime + i * intervalMs),
      hours: value
    }))
  ...
  const [currentData, setCurrentData] = useState({
    time_since_last_visitor:
      patient.patient_score_fields.time_since_last_visitor[index],
    time_since_last_cam_test:
      patient.patient_score_fields.time_since_last_cam_test[index],
  ...
  useEffect(() => {
    ...
    setCurrentData({
      ...
      sleep_deprivation: patient.patient_score_fields.sleep_deprivation[index],
      body_weight_change:
        patient.patient_score_fields.body_weight_change[index],
      hydration_levels: patient.patient_score_fields.hydration_levels[index],
      patient_score: patient.patient_score[index]
    });
    ...
    setTrendData((prev) => {
      const newTrend = [
        ...prev,
        { time: newTime, score: patient.patient_score[index] }
      ];
    ...
    setWeightData((prev) => {
      const newWeight = [
        ...
        {
          time: newTime,
          value: patient.patient_score_fields.body_weight_change[index]
        }
      ];
    ...
    setHydrationData((prev) => {
      const newHydration = [
        ...
        {
          time: newTime,
          value: patient.patient_score_fields.hydration_levels[index]
        }
      ];
      return newHydration.length > maxPoints
        ? newHydration.slice(1)
        : newHydration;
    });
    ...
    setSleepData((prev) => {
      const newSleep = [
        ...
        {
          time: newTime,
          hours: patient.patient_score_fields.sleep_deprivation[index]
        }
      ];
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex justify-between">
          ...
          <span
            className={`p-2 w-11 h-11 text-center rounded-full ${currentData.patient_score < 4 ? 'bg-blue-200' : currentData.patient_score < 8 ? 'bg-yellow-200' : 'bg-orange-200'}`}
          >
            {currentData.patient_score}
          </span>
        </CardTitle>
      </CardHeader>
      <CardContent>
        <div className="grid grid-cols-2 gap-4 p-4">
          <Card
            className={`bg-opacity-10 ${currentData.time_since_last_visitor > 120 ? 'bg-orange-400' : currentData.time_since_last_visitor > 60 ? 'bg-yellow-400' : 'bg-blue-400 '}`}
          >
            <CardContent>
              <h2 className="text-md my-2">Time Since Last Visitor</h2>
          ...
          <Card
            className={`bg-opacity-10 ${currentData.time_since_last_cam_test > 720 ? 'bg-orange-400' : currentData.time_since_last_cam_test > 360 ? 'bg-yellow-400' : 'bg-blue-400 '}`}
          >
            <CardContent>
              <h2 className="text-md my-2">Time Since Last CAM</h2>
              <p className="text-3xl font-bold">
                {currentData.time_since_last_cam_test} min
              </p>
            </CardContent>
          </Card>
              ...
              <ResponsiveContainer width="100%" height={250}>
                <BarChart data={sleepData}>
                  ...
                  <Bar
                    dataKey="hours"
                    fill={
                      sleepData[sleepData.length - 1].hours < 5
                        ? '#ef4444'
                        : '#4ade80'
                    }
                  />
              ...
              <ResponsiveContainer width="100%" height={250}>
                <LineChart data={weightData}>
                  ...
                  <Line
                    ...
                    dataKey="value"
                    stroke={
                      weightData[weightData.length - 1]?.value >= 0
                        ? '#4ade80'
                        : '#ef4444'
                    }
                    strokeWidth={2}
